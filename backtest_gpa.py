...
initial = chart.get_month_chart(2019, 3)['close']
...
def run(year: int, q_size: int):
    ...
    before = before.filter(items=initial.index, axis=0)  # fixme
    ...
    for code in df[0:10].sort_values(by='change', ascending=False).index.tolist():
