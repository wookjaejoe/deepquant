...
class OpenDartRequest:
    ...
    def get(cls, path: str, **params):
        for i in range(3):
            api_key = OpenDartApiKey.next()
            params["crtfc_key"] = api_key
            params_str = "&".join([f"{k}={v}" for k, v in params.items()])
            res = requests.get(f"https://opendart.fss.or.kr/api/{path}?{params_str}")
            if path.endswith("xml") and "<status>020</status>" in str(res.content).replace(" ", "").replace("\n", ""):
                OpenDartApiKey.remove(api_key)
                continue
            elif path.endswith("json") and res.json()["status"] in ["010", "011", "020"]:
                OpenDartApiKey.remove(api_key)
                continue

            return res

        raise RuntimeError("Cannot request to opendart.")
